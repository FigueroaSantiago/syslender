...
if ($es_cobrador) {
    ...
    $query_ruta = $pdo->prepare("
        ...
        FROM ruta r
        JOIN rol_user ru ON ru.id_rol_user = r.id_cobrador
        WHERE ru.id_user = ?
    ");
...
$query_permisos = $pdo->prepare("
    ...
    WHERE pr.id_rol = ?
");
$query_permisos->execute([$user['id_rol_user']]);
                        ...
                        <ul class="list-group">
                            <?php foreach ($gastos as $gasto): ?>
                                <li class="list-group-item">
                                    <strong><?= $gasto['descripcion'] ?>:</strong> €<?= number_format($gasto['monto'], 2) ?> - <em><?= $gasto['fecha'] ?></em>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    <?php else: ?>
